...
export const VERSION_NAMES: Record<number, string> = {
  1: 'V1: Classic Raymarched Cavern',
          ...
          <div className="flex items-center gap-1.5 md:gap-2">
            {[1, 2, 3, 4, 5].map((v) => (
              <button
                ...
                aria-label={`Select Shader Version ${v}: ${VERSION_NAMES[v]}`}
                className={`w-7 h-7 md:w-8 md:h-8 rounded-full text-[9px] md:text-[10px] font-bold transition-all cursor-pointer ${
                  config.shaderVersion === v
                    ? 'bg-zinc-200 text-black shadow-[0_0_12px_rgba(255,255,255,0.3)] scale-105'
      ...
      <div className="bg-[#0a0a0a]/40 border border-zinc-800/60 rounded-full p-2 flex items-center shadow-2xl backdrop-blur-md overflow-x-auto no-scrollbar max-w-full">
        ...
        <div className="hidden lg:flex items-center pr-4 shrink-0">
          <div className="flex flex-col items-end justify-center gap-0.5">
            ...
            <span className="text-zinc-400 text-[7px] md:text-[8px] tracking-[0.2em] uppercase font-bold leading-none mt-1">
